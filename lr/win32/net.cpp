...
int socket_create(void *param) {
  ...
  case Reactor::PROTOCOL_UDP:
    if (scp->overlapped) {
      socket = WSASocket(AF_INET, SOCK_DGRAM,  IPPROTO_UDP, NULL, 0, (scp->overlapped) ? WSA_FLAG_OVERLAPPED: 0);
    } else {
...
int _tcp_socket_send(int socket, struct socket_send_param *ssp) {
  ...
  if (ssp->overlapped) {
    ...
    ret = ::send(socket, ssp->buf, ssp->blen, 0);
    sent = ret;
  }
...
int _udp_socket_send(int socket, struct socket_send_param *ssp) {
  ...
  while ( true ) {
    if (ssp->overlapped) {
      ...
      ret = ::sendto(socket, ssp->buf, ssp->blen, 0, (struct sockaddr*)&addrin, sizeof(addrin));
      sent = ret;
    }
    if ( ret == SOCKET_ERROR) {
      if (WSA_IO_PENDING == WSAGetLastError()) {
